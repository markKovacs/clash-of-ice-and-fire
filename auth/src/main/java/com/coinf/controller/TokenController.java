...
public class TokenController {
    ...
    @ResponseBody
    public void revokeToken2(HttpServletRequest request, @PathVariable String tokenId) {
    ...
    @RequestMapping(method = { RequestMethod.GET}, value = "/oauth/token")
    public boolean revokeToken(HttpServletRequest request) {
        System.out.println("IN DELETING METHOD2");
        String authorization = request.getHeader("Authorization");
        if (authorization != null && authorization.contains("Bearer")) {
            String tokenId = authorization.substring("Bearer".length() + 1);
            System.out.println("DELETE TOKEN2: " + tokenId);
